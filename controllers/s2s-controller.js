...
const data = new Data(trails, textMaxLength);
...
async function train() {
    const model = createModel(textMaxLength, parseInt(data.w2vModel.size), 128, data.pointSize);
    compileModel(model, 1e-2);
    const d = await data.prepareData();
    await fitModel(
        model, d, 200, 128, 0.0625,
        {
